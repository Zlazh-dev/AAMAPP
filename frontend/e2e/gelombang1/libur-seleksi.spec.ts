...
/**
 * Gelombang 1 — spec (c): libur-seleksi.
 * Klik 3 tanggal (toggle seleksi, TANPA membuka dialog langsung — pola
 * rev.2 §14.10.4) → bar aksi "3 tanggal terpilih" muncul → "Tandai Libur"
 * → isi keterangan → tersimpan (sel berubah merah) → regresi "+ Rentang":
...
test('libur: seleksi-multi tandai + regresi tombol + Rentang', async ({ page, request }) => {
  ...
  // Tandai Libur (5) → isi keterangan → simpan.
