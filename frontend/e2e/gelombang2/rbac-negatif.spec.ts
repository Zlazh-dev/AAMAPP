 ...
 * `kurikulum@test.com` â€” TIDAK BENAR (seed hanya membuat 1 admin). Pada
 ...
 * email per run) + hapus di afterEach â€” self-contained & idempoten
 * (Â§12.17e).
 */
test.describe('RBAC Negatif (Poin 10 T16)', () => {
  ...
  test('Kurikulum login -> dapat Data Orang & Kelas, ditolak /admin/akun', async ({ page, request }) => {
    ...
    // Kurikulum punya ADMIN_EXTRA_AREAS=['kurikulum'] â†’ grup kurikulum tampil
    await page.waitForTimeout(1500);
    const hasDataOrang = await page.getByRole('link', { name: 'Data Orang' }).first().isVisible().catch(() => false);
    // Data Orang kini di grup KURIKULUM â€” visible untuk staf kurikulum
    ...
    // 3. Akses /admin/akun (admin-only) â†’ harus redirect atau denied
    ...
    // 4. Akses /admin/sekolah (admin-only) â†’ harus redirect atau denied
    await page.goto('/admin/sekolah');
    await page.waitForTimeout(1000);
    const sekolahUrl = page.url();
    // IA-HIERARCHY-V2 revisi 2026-07-20: /admin/sekolah redirect ke
    // /tu/pengaturan/sekolah (milik TU). Kurikulum tak boleh render halaman.
    const hasSekolahPage = sekolahUrl.includes('/sekolah') && await page.getByText(/Profil Sekolah/i).isVisible().catch(() => false);
    ...
    // email/NIP) â€” bukan 422 lagi. Spec menerima 409.
