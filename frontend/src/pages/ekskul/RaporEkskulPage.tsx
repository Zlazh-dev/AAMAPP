...
export function RaporEkskulPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.show('error', err instanceof ApiError && err.body?.message ? err.body.message : 'Gagal memuat rapor ekskul.');
    } finally {
      setLoading(false);
    }
  }, [siswaId, semester]);
    ...
    <PageContainer>
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <div>
          ...
          <p className="text-sm text-aam-text-muted">Penilaian kegiatan ekskul yang diikuti siswa.</p>
        </div>
                ...
                <Card>
                  <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
                    <div>
                      ...
                      {e.pembinaNama && <p className="text-xs text-aam-text-muted">Pembina: {e.pembinaNama}</p>}
                    </div>
                    {e.kehadiranPct !== null && (
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-aam-text-muted">Kehadiran:</span>
                        <Badge variant={isMerah ? 'red' : 'green'}>{e.kehadiranPct}%</Badge>
                        {isMerah && <span className="material-symbols-outlined text-red-600" style={{ fontSize: '1rem' }}>warning</span>}
                      </div>
                    )}
                  </div>
                  <div className="overflow-x-auto mb-3">
                    <table className="text-sm w-full">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-3 py-2 text-left text-aam-text-muted font-semibold border-b">Tujuan</th>
                          <th className="px-3 py-2 text-center text-aam-text-muted font-semibold border-b">Nilai</th>
                        </tr>
                      </thead>
                  ...
                  {e.deskripsiAuto && (
                    <p className="text-xs text-aam-text-muted italic border-t pt-2">{e.deskripsiAuto}</p>
                  )}
                </Card>
              </div>
            );
