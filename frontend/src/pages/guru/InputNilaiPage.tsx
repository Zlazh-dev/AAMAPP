...
export function InputNilaiPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.show('error', 'Gagal memuat data nilai.');
    } finally {
      setLoading(false);
    }
  }, [penilaianId]);
    ...
    <PageContainer>
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <div>
          ...
          {info && (
            <p className="text-sm text-aam-muted">{info.jenis} · Bobot {info.bobot} · {info.tanggal}</p>
          )}
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-center text-aam-muted font-semibold border-b border-aam-border w-28">Nilai (0–100)</th>
                  <th className="px-3 py-2.5 text-left text-aam-muted font-semibold border-b border-aam-border">Catatan</th>
                </tr>
              </thead>
                ...
                {siswaList.map(s => {
                  ...
                  return (
                    <tr key={s.siswaId} className={belumDiisi ? 'bg-yellow-50' : 'hover:bg-gray-50'}>
                      ...
                      <td className="px-3 py-2 text-aam-muted">{s.nis ?? '—'}</td>
