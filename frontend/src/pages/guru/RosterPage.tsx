 ...
 * Guard: hari ini & belum hadir sesi (hadirPada null) → layar blokir, arahkan ke /guru/kbm.
 */
export function RosterPage() {
  ...
  /** true = hari ini tapi belum hadir sesi */
  ...
  const loadRoster = async () => {
    ...
    try {
      ...
      // Guard: bila hari ini dan belum hadir sesi → blokir
      const isHariIni = tanggal === todayWIB();
      if (isHariIni && !result.hadirPada) {
        setTerblokir(true);
        setLoading(false);
        return;
      }
  ...
  // Guard: hari ini tapi belum hadir sesi
  if (terblokir) {
    return (
      <PageContainer size="lg">
        <BackLink to="/guru/kbm" label="KBM Hari Ini" className="mb-4" mobileButton={false} />
        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center px-4">
          <span className="material-symbols-outlined text-6xl text-yellow-400">location_off</span>
          <h2 className="text-lg font-semibold text-aam-text">Hadir Dulu Sebelum Membuka Roster</h2>
          <p className="text-sm text-aam-text-muted max-w-sm">
            Anda harus menekan tombol <strong>"Hadir &amp; Mulai"</strong> di halaman KBM Hari Ini
            untuk memverifikasi lokasi sebelum dapat mengisi presensi siswa.
          </p>
          <button
            id="btn-kembali-kbm-dari-roster"
            onClick={() => navigate('/guru/kbm')}
            className="rounded-md bg-aam-green px-6 py-3 text-sm font-semibold text-white hover:bg-aam-green/90"
          >
            Ke KBM Hari Ini
          </button>
        </div>
  ...
  return (
    <PageContainer size="lg" bottomBar>
      ...
      <Card flush icon="groups" className="mb-4">
      ...
      {/* Ringkasan status */}
      <Card icon="summarize" className="mb-4">
      ...
      {/* Desktop: tombol inline. Mobile: sticky bottom bar */}
