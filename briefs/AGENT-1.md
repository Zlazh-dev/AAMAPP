...
3. **(C) Hierarki**: admin sidebar → **Dashboard · Data Orang · Kelas · Laporan
   · Pengaturan · Akun**. Buat **Laporan HUB** (`/admin/laporan`) pakai
   **SubPageLinks** ke: Presensi Siswa, Presensi Guru, Izin Guru, Lap Harian
   ...
   Izin Guru dari sidebar → sub Laporan (route tetap ada, cuma keluar sidebar +
   masuk hub). Tiap sub → BackLink ke Laporan. Kepsek: samakan.
...
DoD Gel-1: tsc bersih • build sukses • sidebar 6-item + Laporan hub jalan •
kiosk hilang bersih • validasi wajah di detail guru • e2e MANDIRI hijau •
laporan. Gel-2 (BackLink/SubPageLinks audit semua halaman + card watermark +
emoji→ikon + desktop bottom-sheet fix) tunggu instruksi berikut.

---
## ARSIP TUGAS (2026-07-19c) — F6-INTEGRASI FRONTEND (SELESAI, diterima 8a1a1f7)
...
1. Perluas `RaporDetailPage`: tampilkan 3 bagian dari respons rapor lengkap —
   **akademik** (sudah ada) + **kokurikuler** (per dimensi: nilai+deskripsi) +
   **ekstrakurikuler** (per ekskul: kehadiran% [merah<70%] + tujuan×nilai +
   deskripsi).
2. **PDF rapor PENUH** (perluas `lib/exportPdf.ts`, pdfmake LAZY, kop sekolah):
   satu dokumen rapi — identitas siswa → nilai mapel+deskripsi → kehadiran
   S/I/A → kokurikuler → ekstrakurikuler → catatan wali → tanda tangan.
3. E2E MANDIRI: rapor detail render 3 bagian; tombol export PDF ada; lib PDF
   tetap lazy (bukan main bundle).

DoD: tsc bersih • build sukses • rapor 3 bagian + PDF penuh jalan • export
lazy • e2e hijau • laporan. JANGAN sentuh backend rapor (AG-2).

---
## ARSIP TUGAS (2026-07-19b) — F6d FRONTEND (SELESAI, diterima commit 42630b9)
